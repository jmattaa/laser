...
static void ensure_staat(struct laser_dirent *ent, const char *fp)
{
    //     if (ent->stat_loaded)
    //         return;
    // ent will be reused so loading stat will only use stat of the first entry

    if (lstat(fp, &ent->s) == -1)
        luaL_error(L, "couldn't stat %s", ent->d->d_name);
